...
st.sidebar.markdown("This tool requires your own OpenAI API key to function.")
...
def get_embeddings(texts: List[str], _client) -> np.ndarray:
...
EMBEDDING_MODEL = 'text-embedding-3-small'  # You can also use 'text-embedding-3-large' or 'text-embedding-ada-002'
...
if st.button("Find Similar Questions", type="primary", use_container_width=True):
    ...
    if not master_questions or not generated_questions:
        ...
        with st.spinner("Calculating embeddings and similarities using your API key..."):
            master_embeddings = get_embeddings(master_questions, client)
            generated_embeddings = get_embeddings(generated_questions, client)
