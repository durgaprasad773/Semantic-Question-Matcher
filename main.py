...
def chunk_text(text: str, max_tokens: int = 8000) -> List[str]:
    """Split long text into smaller chunks safely within token limit."""
    words = text.split()
    chunks, chunk, tokens = [], [], 0
    for word in words:
        tokens += len(word) / 4  # rough token estimate
        if tokens >= max_tokens - 500:  # buffer margin
...
def get_embeddings(_client, texts: List[str], model: str) -> np.ndarray:
    """Generate embeddings safely with chunking for long inputs."""
    ...
    try:
        for text in texts:
            ...
            est_tokens = len(text) / 4  # rough token count

            if est_tokens > limit:
                st.warning(
                    f"⚠️ A long question was automatically split "
                    f"(approx {int(est_tokens)} tokens)."
                )
                chunks = chunk_text(text, limit)
                chunk_embeddings = []
                for c in chunks:
                    resp = _client.embeddings.create(input=c, model=model)
